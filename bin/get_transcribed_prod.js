...
const q1 = `select * from content_staging where id >= 106`;
const q2 = `select * from exams_staging where id >= 106`;
const q3 = `select * from images_staging where id >= 1016`;
...
async.parallel([
  ...
  async.series([
    //(outerCallback) => async.each(r2.rows, (row, callback) => local_client.query(i2, [row.id, row.courseid, row.examtype, row.examid, row.profs, row.schoolid, row.datetime], callback), outerCallback),
    ...
    (outerCallback) => async.each(r3.rows, (row, callback) => local_client.query(i3, [row.id, row.examid, row.url]), outerCallback),
  ], (err, results) => {
    if (err) return console.error(err);
